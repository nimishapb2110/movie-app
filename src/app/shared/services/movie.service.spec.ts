...
describe('MovieService', () => {
    ...
    it('should return the movie array', async () => {
        let movieServiceMock = spyOn(movieService, 'getTopMoviesForSearch').and.callFake(() => {
    ...
    it('should return the movie details', async () => {
        let movieServiceMock = spyOn(movieService, 'getMovieDetailsForId').and.callFake(() => {
            return Rx.of(movieMock).pipe(Rx.delay(1000));
        });

        movieService.getMovieDetailsForId('tt11347692').subscribe((response: any) => {
            expect(response).toEqual(Rx.of(movieMock));
        });
